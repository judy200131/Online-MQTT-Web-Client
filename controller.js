...
function connectFunc(){
    ...
    client.on("message", function (topic, payload) {
      document.getElementById("incoming").innerHTML += `<tr><td>${pub_topic.value}</td><td>${pub_payload.value}</td><td>${date.toDateString()+" "+ date.toLocaleTimeString()}</td></tr>`
    })
  ...
  function subscribeFunc(){
    ...
    client.subscribe(pub_topic.value, date)
    document.getElementById("subscribe").innerHTML += `<tr><td>${pub_topic.value}</td><td>${date.toDateString()+" "+ date.toLocaleTimeString()}</td></tr>`
